...
v1.0.12 为 Windows 提供标准 installer。安装后可从开始菜单或桌面快捷方式启动，以后直接运行新版 installer 即可覆盖升级；用户数据不会随程序更新被删除。Windows 设置页还可以手动选择数据存储目录并迁移现有数据。

## v1.0 功能

- 今日净能量：摄入 − 基础消耗 − 运动消耗，清楚展示热量增加或热量缺口。
- 基础消耗快照：同一天使用最后一次保存的数值，今天以前的记录不会被后续修改影响。
- 营养目标：碳水、蛋白质和脂肪按早餐、午餐、晚餐、加餐分色显示完成度。
- 菜谱与记录：支持自定义分类和图片；每日餐食显示菜谱缩略图、餐次、份数、能量与三大营养素。
- 临时餐食：外出就餐时可直接填写名称和营养数据，只记录到当天而不加入菜谱库。
- 放纵日：当天不统计饮食、营养和运动收支，净能量默认按平衡的 0 kcal 计入趋势。
